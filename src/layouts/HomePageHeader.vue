<template>
  <header
    class="fixed top-0 left-0 right-0 z-50 bg-[var(--bg-card)]/80 backdrop-blur-xl transition-all duration-300">
    <div class="max-w-7xl mx-auto px-6 h-[76px] flex items-center justify-between">
      ...
      <nav class="hidden lg:flex items-center gap-8">
        <a href="/" class="text-[14px] font-medium text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors">Trang chủ</a>
        <a href="/khoa-hoc" class="text-[14px] font-medium text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors">Khóa học</a>
      </nav>
      ...
      <div class="flex items-center gap-6">
        ...
        <button 
          @click="toggleDark" 
          class="text-[var(--text-secondary)] hover:text-orange-400 transition-colors p-2"
          aria-label="Toggle Theme"
        >
          <i :class="isDark ? 'fa-solid fa-sun' : 'fa-solid fa-moon'" class="text-lg"></i>
        </button>

        <button class="flex items-center gap-2 text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors group">
          ...
          <div class="relative">
            ...
            <span class="absolute -top-1 -right-1 w-3 h-3 bg-orange-500 border border-[var(--bg-card)] rounded-full"></span>
          </div>
        </button>

        <div v-if="isAuthenticated" class="relative group">
          <button class="flex items-center gap-2.5 text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors">
            <div
              class="w-8 h-8 rounded-full bg-gradient-to-r from-blue-600 to-indigo-600 flex items-center justify-center text-white font-bold text-[13px] shadow-lg">
          ...
          <div
            ...
            <div
              ...
              <a href="/thong-tin-tai-khoan" class="px-4 py-2.5 text-[13px] font-medium text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-white/5 transition-colors flex items-center gap-3">
                <i class="fa-regular fa-user text-gray-400 w-4"></i> Hồ sơ cá nhân
              </a>
              <button @click="handleLogout" class="w-full text-left px-4 py-2.5 text-[13px] font-medium text-red-400 hover:bg-red-500/10 transition-colors flex items-center gap-3">
        ...
        <div v-else>
          <a href="/dang-nhap" class="flex items-center gap-2 text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors group">
