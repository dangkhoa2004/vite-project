<template>
    <div :class="['group flex bg-[var(--bg-card)]/60 backdrop-blur-md rounded-[20px] border border-[var(--border-color)] overflow-hidden hover:border-blue-500/30 hover:shadow-[0_8px_30px_rgba(59,130,246,0.08)] transition-all duration-300',
        layout === 'list' ? 'flex-col md:flex-row p-4 gap-6' : 'flex-col p-3 gap-4 h-full']">

        <div :class="['rounded-xl overflow-hidden shrink-0 relative border border-[var(--border-color)] group transform-gpu bg-[var(--bg-app)]',
            ...
            <div class="absolute top-2 left-2 z-20 flex flex-col gap-1.5 pointer-events-none">
                <span v-for="badge in course.badges" :key="badge" :class="['px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider text-[var(--text-primary)] rounded shadow-sm',
            ...
            <div
                class="absolute inset-0 bg-[var(--bg-app)]/95 backdrop-blur-md opacity-0 group-hover:opacity-100 transition-all duration-300 p-4 flex flex-col justify-center z-10 rounded-xl">
                <p
                    class="text-[11px] font-bold text-white mb-2 border-b border-[var(--border-color)] pb-1.5 uppercase tracking-wider">
                    Bạn sẽ học được:</p>
                <ul class="text-[11px] text-white space-y-1.5">
        ...
        <div class="flex-1 flex flex-col justify-between min-w-0 py-1">
            <div>
                <div class="flex items-center justify-between mb-2">
                    <div class="flex items-center text-yellow-500 text-[11px] gap-0.5">
                        ...
                        <span class="text-[var(--text-secondary)] ml-1">({{ course.reviews }})</span>
                    </div>
                    <span
                ...
                <p v-if="layout === 'list'"
                    class="text-[12px] text-[var(--text-secondary)] line-clamp-2 leading-relaxed mb-3">
                    {{ course.description }}
                </p>

                <div class="flex flex-wrap gap-1.5 mb-3">
                    <span v-for="tool in course.tools" :key="tool"
            ...
            <div
                :class="['flex items-center justify-between text-xs text-[var(--text-secondary)] pt-3 border-t border-[var(--border-color)]', layout === 'grid' ? 'mt-auto' : '']">
                <div class="flex items-center gap-2">
                    <img :src="course.instructor.avatar"
                        class="w-6 h-6 rounded-full object-cover border border-[var(--border-color)]" />
                    <span class="text-[11px] font-medium text-[var(--text-primary)]">{{ course.instructor.name }}</span>
                </div>
                <div class="flex items-center gap-3">
                    <span title="Học viên" class="flex items-center gap-1.5"><i
                            class="fa-solid fa-users text-[10px]"></i> {{ course.students }}</span>
                </div>
            </div>
        </div>

        <div
            :class="['flex justify-between items-center gap-3 border-[var(--border-color)]',
            ...
            <div :class="['flex flex-col', layout === 'list' ? 'text-left md:text-right' : 'text-left']">
                <span v-if="course.price > 0" class="text-[11px] text-[var(--text-secondary)] line-through mb-0.5">
                    {{ formatPrice(course.price * 1.3) }}
                </span>
                <span
                    :class="['text-[15px] font-extrabold tracking-wide leading-none', course.price === 0 ? 'text-emerald-400' : 'text-[var(--text-primary)]']">
