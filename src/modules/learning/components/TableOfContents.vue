<template>
  <div class="bg-[#1c1e24]/80 backdrop-blur-xl rounded-[24px] h-full flex flex-col shadow-2xl border border-[var(--border-color)]">
    ...
    <ul class="flex-1 overflow-y-auto p-3 m-0 flex flex-col gap-2 custom-scrollbar">
      ...
      >
        <div class="flex items-start gap-3">
          <div :class="['w-8 h-8 rounded-full flex items-center justify-center shrink-0 mt-0.5 transition-colors', store.currentLesson.id === lesson.id ? 'bg-white/20 text-[var(--text-primary)]' : 'bg-[#2a2d35] text-[var(--text-secondary)] group-hover:bg-[#323640]']">
            <i :class="index < 2 ? 'fa-solid fa-check text-[12px]' : 'fa-solid fa-play text-[10px] ml-0.5'"></i>
          </div>
          
          <div class="flex-1 min-w-0">
            <div :class="['text-[13px] font-medium truncate mb-1', store.currentLesson.id === lesson.id ? 'text-[var(--text-primary)]' : 'text-gray-300 group-hover:text-[var(--text-primary)]']">
              {{ lesson.title }}
            </div>
