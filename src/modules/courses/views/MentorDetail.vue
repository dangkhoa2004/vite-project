...
<template>
    ...
    <div class="min-h-screen bg-[#0f1115] text-gray-200 font-sans relative overflow-hidden pb-20 pt-24">
        ...
        <main class="relative z-10 max-w-7xl mx-auto px-6">
            ...
            <!-- Cover Banner -->
            <div
                class="w-full h-48 md:h-64 rounded-[24px] overflow-hidden relative border border-white/5 shadow-lg mt-4">
                <img :src="mentor.cover" alt="Cover" class="w-full h-full object-cover opacity-60" />
                <div class="absolute inset-0 bg-gradient-to-t from-[#0f1115] via-[#0f1115]/50 to-transparent"></div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-[320px_1fr] gap-8 items-start -mt-20 md:-mt-24 relative z-20">
                ...
                <aside class="space-y-6">
                    <!-- Profile Card -->
                    <div
                        class="bg-[#1c1e24]/80 backdrop-blur-xl rounded-[24px] border border-white/5 p-6 flex flex-col items-center text-center shadow-[0_15px_40px_rgba(0,0,0,0.5)] relative overflow-hidden">
                        ...
                        <div
                            ...
                            <img :src="mentor.avatar" alt="Avatar"
                                class="w-full h-full rounded-full object-cover border-4 border-[#1c1e24]" />
                        </div>
                        ...
                        <!-- Quick Stats -->
                        <div class="flex items-center justify-center gap-6 w-full border-y border-white/5 py-4 mb-5">
                            <div class="flex flex-col items-center">
                                <span class="text-lg font-bold text-white">{{ formatNumber(mentor.totalStudents)
                                    }}</span>
                        ...
                        <div class="w-full space-y-3">
                            <button
                                class="w-full py-3 bg-blue-600 hover:bg-blue-500 text-white text-[13px] font-bold rounded-xl shadow-[0_4px_15px_rgba(37,99,235,0.3)] transition-all flex items-center justify-center gap-2">
                                <i class="fa-solid fa-plus text-[12px]"></i> Theo dõi giảng viên
                            </button>
                            <button
                                class="w-full py-3 bg-white/5 hover:bg-white/10 border border-white/10 text-white text-[13px] font-bold rounded-xl transition-all flex items-center justify-center gap-2">
                    ...
                    <!-- Skills & Tech Stack -->
                    <div class="bg-[#1c1e24]/60 backdrop-blur-xl rounded-[24px] border border-white/5 p-6">
                        <h3
                            class="text-[13px] font-bold text-white uppercase tracking-wider mb-4 border-b border-white/5 pb-3">
                            Kỹ năng chuyên môn</h3>
                        <div class="flex flex-wrap gap-2">
                            <span v-for="skill in mentor.skills" :key="skill"
                                class="px-3 py-1.5 bg-black/30 border border-white/10 rounded-lg text-[11px] text-gray-300 font-medium hover:border-blue-500/50 hover:text-white transition-colors cursor-default">
                                {{ skill }}
                            </span>
                        </div>
                    </div>
                ...
                <div class="space-y-6 md:mt-24 lg:mt-0">
                    ...
                    <!-- Tab Navigation -->
                    <div
                        class="flex bg-[#1c1e24]/80 p-1.5 rounded-2xl border border-white/5 shadow-inner w-full overflow-x-auto custom-scrollbar backdrop-blur-xl">
                    ...
                    <Transition name="fade" mode="out-in">
                        <div v-if="activeTab === 'about'" class="space-y-6">
                            <!-- About Bio -->
                            <div
                                class="bg-[#1c1e24]/60 backdrop-blur-xl rounded-[24px] border border-white/5 p-6 md:p-8">
                                <h2 class="text-lg font-bold text-white mb-4">Về giảng viên</h2>
                            ...
                            <div
                                class="bg-[#1c1e24]/60 backdrop-blur-xl rounded-[24px] border border-white/5 p-6 md:p-8">
                                <h2 class="text-lg font-bold text-white mb-6">Kinh nghiệm làm việc</h2>
                                <div
                                    ...
                                    <div v-for="(exp, index) in mentor.experiences" :key="index"
                                        ...
                                        <div
                                            class="flex items-center justify-center w-10 h-10 rounded-full border-4 border-[#0f1115] bg-[#1c1e24] group-hover:bg-blue-600 group-hover:border-blue-500/30 text-gray-400 group-hover:text-white shadow shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 transition-colors z-10 ml-0 md:ml-0">
                                            <i class="fa-solid fa-briefcase text-[12px]"></i>
                                        </div>

                                        <div
                                            class="w-[calc(100%-3rem)] md:w-[calc(50%-2.5rem)] bg-black/20 p-5 rounded-2xl border border-white/5 hover:border-white/10 transition-colors ml-4 md:ml-0">
                                            <div class="flex items-center justify-between mb-1">
                                                <h3 class="font-bold text-white text-[14px]">{{ exp.role }}</h3>
                                                <span
                                                    class="text-[10px] font-bold text-blue-400 bg-blue-500/10 px-2 py-0.5 rounded">{{
                                                    exp.year }}</span>
                                            </div>
                                            <p class="text-[12px] text-gray-400 mb-2 font-medium">{{ exp.company }}</p>
                        ...
                        <div v-else-if="activeTab === 'courses'" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div v-for="course in mentorCourses" :key="course.id"
                                class="group flex flex-col bg-[#1c1e24]/60 backdrop-blur-md rounded-[20px] border border-white/5 overflow-hidden hover:border-blue-500/30 hover:shadow-[0_8px_30px_rgba(59,130,246,0.08)] transition-all duration-300 p-3 gap-4">
                                <div
                                    class="w-full h-40 rounded-xl overflow-hidden relative border border-white/5 bg-[#0f1115] transform-gpu">
                                ...
                                <div class="flex-1 flex flex-col justify-between py-1">
                                    <div>
                                        ...
                                        <h3
                                            class="text-[14px] font-bold text-white mb-2 group-hover:text-blue-400 transition-colors line-clamp-2">
                                            {{ course.title }}</h3>
                                    </div>
                                    <div class="flex justify-between items-center pt-3 border-t border-white/5 mt-auto">
                                        <span
                                            :class="['text-[14px] font-extrabold', course.price === 0 ? 'text-emerald-400' : 'text-white']">
                                            {{ course.price === 0 ? 'Miễn phí' : formatPrice(course.price) }}
                                        </span>
                                        <button
                                            class="px-4 py-2 bg-white/5 hover:bg-blue-600 text-white text-[11px] font-bold rounded-lg transition-colors">Xem
                                            chi tiết</button>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div v-else-if="activeTab === 'reviews'" class="space-y-4">
                            <div
                                class="flex items-center justify-between bg-[#1c1e24]/60 p-5 rounded-[20px] border border-white/5 mb-6">
                                <div class="flex items-center gap-4">
                                    <div class="text-4xl font-extrabold text-white">{{ mentor.rating }}</div>
                                    <div class="flex flex-col">
                                        <div class="flex text-yellow-500 text-[13px] mb-1">
                                            <i v-for="i in 5" :key="i" class="fa-solid fa-star"></i>
                                        </div>
                                        <span class="text-[11px] text-gray-400">Trung bình từ {{ mentor.totalReviews }}
                                            đánh giá</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div v-for="review in reviews" :key="review.id"
                                class="bg-[#1c1e24]/40 p-5 rounded-[20px] border border-white/5 flex gap-4">
                                <img :src="review.userAvatar"
                                    class="w-10 h-10 rounded-full object-cover shrink-0 border border-white/10" />
                                <div>
                                    <div class="flex items-center justify-between mb-1">
                                        <h4 class="text-[13px] font-bold text-white">{{ review.userName }}</h4>
