<template>
  <section class="flex flex-col items-center">
    <div class="text-center mb-10">
      <h2 class="text-3xl font-bold text-[var(--text-primary)] mb-4">Bài học chất lượng cho học viên</h2>
      <p class="text-sm text-[var(--text-secondary)] mb-8 max-w-xl mx-auto">
        Lộ trình học tập được thiết kế chuẩn mực cho từng cấp độ, giúp bạn đi từ cơ bản đến chuyên sâu.
      </p>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 w-full mb-10">
      <div v-for="i in 8" :key="i" class="bg-[var(--bg-card)]/60 backdrop-blur-md rounded-[24px] p-6 border border-[var(--border-color)] hover:-translate-y-2 hover:border-blue-500/30 transition-all duration-300 flex flex-col items-center text-center group">
        ...
        <h3 class="text-lg font-bold text-[var(--text-primary)] mb-2 group-hover:text-blue-400 transition-colors">
          Tiêu chuẩn {{ ['Một', 'Hai', 'Ba', 'Bốn', 'Năm', 'Sáu', 'Bảy', 'Tám'][i-1] }}
        </h3>
        <p class="text-xs text-[var(--text-secondary)] mb-6 flex-1 leading-relaxed">
          Tài liệu chuẩn hóa giúp học viên tiếp thu kiến thức nền tảng một cách dễ dàng và hiệu quả nhất.
        </p>
        
        <button class="w-full py-2.5 rounded-xl border border-[var(--border-color)] text-[var(--text-secondary)] text-sm font-medium hover:bg-[var(--border-color)] transition-colors group-hover:border-blue-500/50 group-hover:text-blue-400">
          Chi tiết lớp học
        </button>
      </div>
    </div>
