<template>
    <div class="relative max-w-4xl mx-auto py-8">
        ...
        <div class="absolute left-[27px] sm:left-1/2 top-0 bottom-0 w-1 bg-white/5 rounded-full transform sm:-translate-x-1/2">
            <div class="absolute top-0 left-0 w-full bg-gradient-to-b from-blue-500 to-purple-500 rounded-full transition-all duration-1000" style="height: 50%;"></div>
        </div>

        <div class="space-y-12 relative">
            <div v-for="(node, index) in roadmapNodes" :key="node.id" 
                :class="['relative flex flex-col sm:flex-row items-start sm:items-center gap-6 sm:gap-12 group', 
                         index % 2 === 0 ? 'sm:flex-row-reverse' : '']">
                
                <div class="absolute left-0 sm:left-1/2 transform sm:-translate-x-1/2 w-14 h-14 flex items-center justify-center z-10 shrink-0">
                    <div :class="['w-10 h-10 rounded-full flex items-center justify-center border-4 transition-all duration-500', 
                        ...
                        <i v-if="node.status === 'completed'" class="fa-solid fa-check text-white text-[14px]"></i>
                        <i v-else-if="node.status === 'active'" class="fa-solid fa-code text-white text-[12px]"></i>
                ...
                <div :class="['w-full sm:w-1/2 pl-16 sm:pl-0', index % 2 === 0 ? 'sm:pr-12' : 'sm:pl-12']">
                    <div :class="['p-6 rounded-[24px] border backdrop-blur-xl transition-all duration-300',
                        ...
                        <div class="flex items-center gap-2 mb-3">
                            ...
                            <span v-if="node.isRequired" class="text-[10px] text-orange-400 bg-orange-400/10 px-2 py-1 rounded-md font-bold">Bắt buộc</span>
                        </div>

                        <h3 :class="['text-lg font-bold mb-2', node.status === 'locked' ? 'text-gray-500' : 'text-white']">{{ node.title }}</h3>
                        <p class="text-[13px] text-gray-400 mb-4 line-clamp-2 leading-relaxed">{{ node.description }}</p>
                        
                        <div v-if="node.prerequisites.length > 0" class="mb-4 p-3 bg-black/40 rounded-xl border border-white/5">
                            <p class="text-[11px] text-gray-500 mb-1 font-semibold uppercase">Kiến thức tiên quyết:</p>
                            <div class="flex flex-wrap gap-2 mt-2">
                                <span v-for="(req, i) in node.prerequisites" :key="i" class="text-[11px] text-gray-300 bg-white/5 px-2 py-1 rounded-md border border-white/5">
                        ...
                        <button v-if="node.status !== 'locked'" 
                            ...
                            node.status === 'completed' ? 'bg-white/5 hover:bg-white/10 text-white' : 'bg-blue-600 hover:bg-blue-500 text-white shadow-lg']">
