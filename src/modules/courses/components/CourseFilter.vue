<template>
    <div
        class="flex flex-col xl:flex-row gap-4 items-start xl:items-center justify-between bg-[var(--bg-app)]/40 backdrop-blur-xl p-3 rounded-[24px] border border-[var(--border-color)]">

        <div
            class="flex bg-[var(--bg-app)] p-1 rounded-xl border border-[var(--border-color)] w-full xl:w-auto overflow-x-auto custom-scrollbar shrink-0">
            <button @click="$emit('update:activeTab', 'learning')"
                :class="['px-4 sm:px-5 py-2 text-[13px] font-bold rounded-lg transition-all whitespace-nowrap flex items-center gap-2',
                    activeTab === 'learning' ? 'bg-blue-600 text-[var(--text-primary)] shadow-[0_4px_12px_rgba(37,99,235,0.3)]' : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)]']">
                <i class="fa-solid fa-fire text-[12px]"></i> Đang học ({{ learningCount }})
            </button>
            <button @click="$emit('update:activeTab', 'completed')"
                :class="['px-4 sm:px-5 py-2 text-[13px] font-bold rounded-lg transition-all whitespace-nowrap flex items-center gap-2',
                    activeTab === 'completed' ? 'bg-emerald-600 text-[var(--text-primary)] shadow-[0_4px_12px_rgba(16,185,129,0.3)]' : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)]']">
                <i class="fa-solid fa-circle-check text-[12px]"></i> Hoàn thành ({{ completedCount }})
            </button>
            <button @click="$emit('update:activeTab', 'certificates')"
                :class="['px-4 sm:px-5 py-2 text-[13px] font-bold rounded-lg transition-all whitespace-nowrap flex items-center gap-2',
                    activeTab === 'certificates' ? 'bg-purple-600 text-[var(--text-primary)] shadow-[0_4px_12px_rgba(147,51,234,0.3)]' : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)]']">
                <i class="fa-solid fa-certificate text-[12px]"></i> Chứng chỉ ({{ completedCount }})
            </button>
            <button @click="$emit('update:activeTab', 'roadmap')"
                :class="['px-4 sm:px-5 py-2 text-[13px] font-bold rounded-lg transition-all whitespace-nowrap flex items-center gap-2',
                    activeTab === 'roadmap' ? 'bg-green-600 text-[var(--text-primary)] shadow-[0_4px_12px_rgba(16,185,129,0.3)]' : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)]']">
        ...
        <div class="flex flex-col sm:flex-row gap-3 w-full xl:w-auto items-center flex-1 justify-end">
            <div class="relative w-full sm:w-[200px]" ref="dropdownRef">
                <button @click="isDropdownOpen = !isDropdownOpen"
                    ...
                    <i
                        :class="['fa-solid fa-chevron-down text-[10px] text-[var(--text-secondary)] transition-transform duration-300 shrink-0', isDropdownOpen ? 'rotate-180' : '']"></i>
                </button>
                <Transition name="dropdown-fade">
                    <div v-if="isDropdownOpen"
                        class="absolute right-0 top-full mt-2 w-full bg-[var(--bg-app)] border border-white/10 rounded-xl shadow-[0_10px_30px_rgba(0,0,0,0.5)] overflow-hidden z-50 py-1.5 backdrop-blur-xl">
                        <ul>
                            <li v-for="option in sortOptions" :key="option.value" @click="selectSort(option.value)"
                                :class="['px-4 py-2.5 text-[13px] cursor-pointer transition-colors flex items-center justify-between',
                                    sortBy === option.value ? 'text-blue-400 bg-[var(--bg-app)] font-bold' : 'text-[var(--text-secondary)] hover:bg-[var(--bg-app)] hover:text-[var(--text-primary)]'
                                ]">
                                {{ option.label }}
